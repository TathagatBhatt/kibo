...
def User_Input_Handler():
    ...
    elif any(phrase in query.lower() for phrase in ['tell me a joke']):
        print(joke)
        speak(joke)
    elif any(phrase in query.lower() for phrase in ['search on google']):
    ...
    elif any(phrase in query.lower() for phrase in ['open wikpedia']):
        speak("opening Wikipedia...", language='en')
        url = 'wikipedia.com'
        webbrowser.get(chrome_path).open(url)
    elif any(phrase in query.lower() for phrase in ['open youtube']):
        speak("opening Youtube...", language='en')
        url = 'youtube.com'
        webbrowser.get(chrome_path).open(url)
    elif any(phrase in query.lower() for phrase in ['open spotify']):
        speak("opening Spotify...", language='en')
        os.system('start spotify://')
    elif any(phrase in query.lower() for phrase in ['play music']):
        song_dir = "C:\\Users\\tatha\\Music\\Music"
        song = os.listdir(song_dir)
        os.startfile(os.path.join(song_dir, song[0]))
    elif any(phrase in query.lower() for phrase in ['time']):
    ...
    elif any(phrase in query.lower() for phrase in ['tell me a fact']):
        speak(r.choice(Fact_lib))
    elif any(phrase in query.lower() for phrase in ['shutdown']):
        speak("shutindown")
        quit()
    elif any(phrase in query.lower() for phrase in ['translator' or 'translater']):
